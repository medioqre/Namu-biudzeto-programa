...
def info():
    messagebox.showinfo('Grupės nariai','Dominykas Vnęk - backend\nNerijus Juodsnukis - grafinė sąsaja\nAndrius Stankevičius - treeview\nKajus Dobrovolskis - grafikai') 
info_myg=Button(left,text='Info', font=('Arial', 12, 'bold'),command=info)
info_myg.grid(row=7, sticky='we',pady=5,padx=5)
...
for i in Skyriai:
    ...
    Tree.column(i, width=80, anchor='center', stretch=True)
...
#Treeview
Skyriai_islaidos=('Pavadinimas', 'Data', 'Kategorija', 'Suma')
...
for col in Skyriai_islaidos:
    ...
    Tree_islaidos.column(col, width=80, anchor='center', stretch=True)
...
tab_balansas.rowconfigure(1, weight=1) 
tab_balansas.rowconfigure(3, weight=3)  
